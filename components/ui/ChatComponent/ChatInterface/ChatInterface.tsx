...
export default function ChatInterface({
  ...
  if (!mounted) {
    return (
      <div className="flex h-[calc(100vh-3.5rem)] sm:h-[calc(100vh-4rem)] md:h-[calc(100vh-5rem)] flex-col px-2 sm:px-4">
  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)] sm:h-[calc(100vh-4rem)] md:h-[calc(100vh-5rem)] flex-col px-2 sm:px-4">
      <div className="flex-1 rounded-lg border border-zinc-800 bg-zinc-950 shadow-xl flex flex-col overflow-hidden">
        {!hasSubscription && (
          <div className="shrink-0 px-2 sm:px-4 pt-2 sm:pt-4 text-xs sm:text-sm border-b border-zinc-800 bg-zinc-900/50">
            {isLimitReached ? (
              <div className="pb-2 sm:pb-4 flex items-center justify-between">
                <span className="text-red-400 text-xs sm:text-sm">
                  0 credits left for today. Check back in a few days, or
                  subscribe for unlimited access.
                </span>
                <button
                  className="ml-2 px-2 sm:px-4 py-1 text-xs sm:text-sm bg-blue-600 hover:bg-blue-500 text-white rounded-md transition-colors whitespace-nowrap"
                  onClick={() => {
              ...
              <div className="pb-2 sm:pb-4 flex items-center justify-between">
                ...
                <button
                  className="ml-2 px-2 sm:px-4 py-1 text-xs sm:text-sm bg-zinc-800 hover:bg-zinc-700 text-zinc-100 rounded-md transition-colors whitespace-nowrap"
                  onClick={() => {
                    router.push('/subscribe');
                  }}
                >
          ...
          <div className="flex flex-col h-full overflow-y-auto">
            {isLoading ? (
              <div className="flex-1 flex flex-col justify-center items-center text-zinc-400">
                ...
                />
                <div className="text-base sm:text-lg text-zinc-600 mt-2">
